...
BEGIN
    ...
    DECLARE auth_status ENUM('N', 'Y');

    -- 환자 인증 상태를 확인합니다.
    SELECT patient_authentication INTO auth_status
    FROM Patients
    WHERE identity_number = identity_number_param;

    IF auth_status = 'N' THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = '사용자 인증 오류입니다.';
    ELSE
        -- 현재 요일을 조회한다.
        ...
        INSERT INTO Waiting (registration_id, doctor_id, patient_id, waiting_count, status, created_time)
        SELECT @last_inserted_id, doctor_id_param, patient_id_param, COALESCE(MAX(waiting_count), 0) + 1, '접수', NOW()
        FROM Waiting;
    END IF;
END;
